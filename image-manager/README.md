...
```bash
sudo mkfs.ext4 -L im-data /dev/sdb && sudo mkdir -p /data && echo "UUID=$(sudo blkid -s UUID -o value /dev/sdb) /data ext4 defaults,noatime 0 2" | sudo tee -a /etc/fstab && sudo systemctl daemon-reload && sudo mount -a && df -h /data
```

**Mounted by UUID, not by `/dev/sdb`.** Device names reorder across reboots, and a wrong-disk mount
on the box everything pulls from is not a failure you want to debug at 3am.

**UUID rather than a LABEL**, and this one is worth knowing: ext4 labels are capped at **16
characters and truncate silently**. A longer name produces a filesystem whose real label does not
match the one written into `fstab`, so the mount fails — and if you only notice at the next reboot,
it fails on a box with no console. A UUID is generated by the filesystem, has no length limit, and
cannot collide with another disk.

The `-L im-data` above is inside the limit and is only a human-readable convenience; the mount does
not depend on it.
